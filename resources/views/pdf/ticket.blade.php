...
<head>
    <meta charset="utf-8">
    <title>Ticket - {{ $orderItem->ticket_code }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="ticket">
            ...
            @if($event->location_type === 'venue' && $event->location)
                <div class="event-location">
                    ...
                    @if(!empty($event->location['address']))
                        <br>{{ $event->location['address'] }}@if(!empty($event->location['city'])), {{ $event->location['city'] }}@endif @if(!empty($event->location['state'])), {{ $event->location['state'] }}@endif
                    @endif
                </div>
        ...
        <div class="section">
            ...
            <div class="attendee-name">{{ $orderItem->attendee_name ?: $order->customer_name }}</div>
        </div>
        ...
        <div class="section">
            ...
            <div class="ticket-type">
                @if($orderItem->ticketTier)
                    {{ $orderItem->ticketTier->name }}
                @elseif($orderItem->table && $orderItem->seat)
                    {{ $orderItem->table->name }} &middot; Seat {{ $orderItem->seat->label }}
                @elseif($orderItem->table)
                    {{ $orderItem->table->name }}
                @else
                    {{ $orderItem->item_name ?: 'General Admission' }}
                @endif
            </div>
        </div>
