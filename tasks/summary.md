...
POST   /checkout/create-session          # Stripe checkout (routed to the event's account)
POST   /webhooks/stripe                  # Stripe webhooks (legacy alias for cafeteria)
POST   /webhooks/stripe/{account}        # Per-account webhooks: cafeteria | rifa | eventos
```
...
# Stripe — three accounts, routed per event via events.stripe_account
# (cafeteria falls back to legacy STRIPE_* vars if the CAFETERIA ones are unset)
STRIPE_CAFETERIA_KEY=pk_test_...
STRIPE_CAFETERIA_SECRET=sk_test_...
STRIPE_CAFETERIA_WEBHOOK_SECRET=whsec_...
STRIPE_RIFA_KEY=...
STRIPE_RIFA_SECRET=...
STRIPE_RIFA_WEBHOOK_SECRET=...
STRIPE_EVENTOS_KEY=...
STRIPE_EVENTOS_SECRET=...
STRIPE_EVENTOS_WEBHOOK_SECRET=...
STRIPE_DEFAULT_ACCOUNT=cafeteria
